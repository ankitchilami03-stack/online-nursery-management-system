...
$ddQuery = mysqli_query($con, "SELECT DeliveryDate FROM tblorderaddresses WHERE UserId='$userid' AND Ordernumber='$oid' LIMIT 1");
$ddResult = mysqli_fetch_array($ddQuery);
$deliveryDate = $ddResult ? date('d-m-Y', strtotime($ddResult['DeliveryDate'])) : 'Not Available';
...
<p style="color:#000"><b>Expected Delivery Date :</b> <?php echo $deliveryDate; ?></p>
...
$cnt=$cnt+1; 
 ...
 ?>
							<?php
							$qtyres = mysqli_fetch_array(mysqli_query($con, "SELECT Quantity FROM tblorderaddresses WHERE OrderNumber='$oid' AND UserId='$userid' LIMIT 1"));
							$quantity = $qtyres ? $qtyres['Quantity'] : 1; // fallback to 1 if not found
							?>
							<td class="qnt"><?php echo $quantity; ?></td>
...
$cnt=$cnt+1; 
 ...
 ?>
							...
							<!--<td class="price">
  <?php 
    $method = strtolower($row['PaymentMethod']);
  ...
  ?>
							...
							<td>
  <a href="pdf.php?pid=<?php echo $row['ID']; ?>">
    <button type="button">Download Instructions</button>
  </a>
</td>
